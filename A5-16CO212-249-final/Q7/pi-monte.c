...
#include <math.h>
#define MOD 10000007
#define MULTIPLIER 1754	
#define INCREMENT 32561
long long seed;
...
double seq_pimonte(long n_steps,int r)
{
	...
	return 4.0*count/n_steps;

}

double parallel_pi_monte(long n_steps,int r,int NUM_THREADS)
{
	double cx =1.0, cy = 1.0;
...
int main()
{
	...
	double time_sequence,time_parallel;
	double pi;

	time_sequence = omp_get_wtime();
	pi = seq_pimonte(n_steps,1);
	...
	while(NUM_THREADS<=20)
	{
		...
		pi = parallel_pi_monte(n_steps,1,NUM_THREADS);
